...
public class BuiltInQProfileDefinitionsBridgeTest {
  ...
  public void bridgeProfileDefinitions() {
    ...
    assertThat(defaultSeverity.overridenSeverity()).isNull();
    assertThat(defaultSeverity.overridenParams()).isEmpty();

    assertThat(profile1.rule(RuleKey.of("repo1", "overrideSeverity")).overridenSeverity()).isEqualTo(Severity.CRITICAL);

    assertThat(profile1.rule(RuleKey.of("repo1", "overrideParam")).overridenParams())
      .extracting(BuiltInQualityProfilesDefinition.OverridenParam::key, BuiltInQualityProfilesDefinition.OverridenParam::overridenValue).containsOnly(tuple("param", "value"));
  }
